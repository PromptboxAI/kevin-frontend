...
/**
 * The sample files are real, generated by `scripts/build-samples.mjs` and
 * served from `public/samples/`. Their money columns are computed with the same
 * chain the server uses, so a sample cannot quietly disagree with the product,
 * and the .xlsx carries static values only (rule 2b — Xactimate's importer
 * breaks on formulas). One row is deliberately unpriced, because that is a
 * normal state and a sample that hides it misrepresents the real file.
 */
const FORMATS: [string, string, string, boolean, string][] = [
  [
    'XLSX',
    'XactContents Excel template',
    'The pre-formatted .xlsx Xactimate imports directly. This is the default.',
    true,
    '/samples/kevin-sample-xactcontents.xlsx',
  ],
  [
    'PDF',
    'PDF inventory',
    ...
    >
      ...
      <section className="k-set-card">
        ...
        <div className="k-set-card-body" style={{ padding: 0 }}>
          {FORMATS.map(([tag, name, desc, primary, sample]) => (
            <div key={tag} className="k-int-row">
              ...
              {/* A real file, so a plain anchor with `download` -- no handler,
                  nothing to fail, and it works with middle-click and
                  right-click-save the way a download should. */}
              <a className="k-btn k-btn--ghost" href={sample} download>
                <Icon d={I.download} size={12} /> Download sample
              </a>
            </div>
          ))}
        </div>
