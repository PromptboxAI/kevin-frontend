...
| 11 Mobile capture | `/capture` (PUBLIC) | ⛔ BLOCKED (ask 32) | Built: shoot via the phone's own camera, per-BATCH room (captured per shot, batched on change), per-photo note (300), real queue states, honest "safe to leave". **Cannot work until `X-Capture-Token` is added to the backend's CORS `allow_headers`** — the preflight is rejected today, so the upload never leaves the browser. Deliberately NOT ported: live viewfinder (costs EXIF the clusterer needs), flash, SCAN BARCODE (Chrome-only), the offline-queue banner (not built — so nothing claims it), and the artboards' fake phone chrome. |
