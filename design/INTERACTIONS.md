...
| 48 Sample-claim | **Sample claim** (footer, every page) · **Open the sample claim** (`/demo`, `/done-for-you`) · **See a finished claim** (landing hero) · **See a sample inventory** (`/for-estate-liquidators`) | ✅ | All four → `/sample`, which redirects to `/claims/sample`. SHIPPED: the fixture is served at the `request()` boundary in `lib/api.ts`, scoped to the sample route, so WorksheetPage is untouched and there is no parallel grid. `BACKEND-ASKS.md` #37 stays open as the long-term answer — when that endpoint lands, deleting `lib/sample*.ts` and six lines of api.ts leaves the same page on real data |
