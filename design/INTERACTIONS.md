...
| 05 Worksheet | **% Depr. cell** | ✅ LIVE | Editable override. Typed as a PERCENT (`55` = 55%); sent as `dep_manual`, a FRACTION — the conversion is the dangerous part, so `parseDeprPercent` owns it and rejects anything outside 0–100 before the API sees it (`55` unconverted would be 5500%). Supplying `dep_manual` flips `depreciation_method` to `custom` server-side, so the method is never sent alongside it. **No client-side math**: `$ Depr.` and `ACV` are the server's recomputed totals, applied verbatim from the same `override` response the Age cell uses; the cell spins until they land. Unpriced (`needs_manual`) rows are disabled — 55% of nothing is nothing. An overridden rate renders in accent (`.k-c--depmanual`), never amber (rule 6): overriding is normal, not a warning. |
