...
|---|---|---|---|
| 02 Landing **Start a new claim** | landing | ✅ wired | Prototype → `00-Sign-in.html`. PRODUCTION: if a session exists, go straight to `/claim/new` (intake); if not, sign-in → (new users) account create + onboarding → intake. Claims are auth-gated; the sample claim is the only unauthenticated claim surface. |
| 02 Landing **Open a sample claim** | landing | ✅ wired | → `48-Sample-claim.html` — public, no auth. |
...
| Page | Control | State | Production behavior |
|---|---|---|---|
| 02 Landing | Hero **Start a new claim** / **Open a sample claim** | ✅ | onStartClaim → intake; onSampleClaim → sample claim |
| 02 | Final CTA **Start your first claim** / **Book a 30-min call** | ✅ / 🔌 | first→intake; Book→ `51-Book-call.html` |
| 20 Product (37) | Hero + CTA **Start a new claim** / **See pricing** | 🔌 | intake / `21-Pricing.html` |
...
| **Export inventory** dropdown → **PDF inventory** | ✅ | → `74-PDF-inventory.html` (estate variant). Direct download, no review modal |
| **Export inventory** → **Excel · .xlsx** | 🔌 | `POST /v1/estates/:id/export {format:'xlsx'}` — downloads immediately. Columns: # · Room/Area · Description · Condition · Fair market value · Sale price · Status · Source link. NO depreciation/ACV/tax columns |
