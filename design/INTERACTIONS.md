...
| 01 | Row ⋯ **menu placement** | ✅ LIVE | **Portalled to `<body>` and `position: fixed`.** It was `position: absolute` inside the row, which cannot escape an overflow ancestor — and `.k-claims-list` is an `overflow: auto` box sized to its rows, so on a short list the menu was clipped to a sliver you had to scroll inside to read. Positioned from the trigger's rect, right-aligned to it: opens below, flips above when the viewport has no room there, and clamps on screen when it fits neither. Repositions on scroll/resize (only when it actually moved — this runs per frame), closes on Escape and outside click, and the outside-click check includes the portalled node or the first click on a menu item would close it before firing. |
...
| 16 | Full-screen viewer | ✅ LIVE | ←/→/Esc plus the header arrows, paging within the current filter. Serves the **original** (4000×3000, via the item detail's `image_url`) when the photo is its line's primary; otherwise the 600×450 thumbnail, labelled *Preview* rather than captioned as full size. There is no per-`photo_id` route to an original — ask 27(a). |
...
| 05 Worksheet | **% Depr. cell** | ✅ LIVE | Editable override. Typed as a PERCENT (`55` = 55%); sent as `dep_manual`, a FRACTION — the conversion is the dangerous part, so `parseDeprPercent` owns it and rejects anything outside 0–100 before the API sees it (`55` unconverted would be 5500%). Supplying `dep_manual` flips `depreciation_method` to `custom` server-side, so the method is never sent alongside it. **No client-side math**: `$ Depr.` and `ACV` are the server's recomputed totals, applied verbatim from the same `override` response the Age cell uses; the cell spins until they land. Unpriced (`needs_manual`) rows are disabled — 55% of nothing is nothing. An overridden rate renders in **accent, colour only** (`.k-c--depmanual`) — never amber (rule 6; overriding is normal, not a warning) and never bold, because bold already marks the ACV bottom line in this grid. |
| 05 | **Back to the schedule** (in *How this was calculated*) | ✅ LIVE | Releases the override — `PATCH …/override {depreciation_method: "straight_line"}`. Releasing must NAME a method: per the contract, sending `custom` with no rate keeps the existing one, so there is no "unset". Straight-line is the product default (rule 13) and nothing on the claim records which table was in use before the override. **It is a button, not "clear the cell"**: focusing a numeric cell blanks it, so there is nothing left to delete and blur restores the old value — an adjuster trying to clear their way out would watch the rate spring back with no explanation. Typing a value then deleting it still works as a second path. |
| 05 Worksheet | **Rooms** (toolbar) | ✅ LIVE | Filter the grid by room or by Unassigned, and create / rename / remove rooms. `GET·POST /claims/{id}/rooms`, `PATCH·DELETE /rooms/{id}`. Duplicate names are caught client-side before the `409`. Removing a room keeps its items (they fall to Unassigned) and keeps their Room/Area text — the copy says so. Unassigned is derived from the items, not counted server-side. |
| 05 | **File in room…** (selection bar) | ✅ LIVE | **Two writes on purpose.** `PATCH /claim_items/assign-room` sets `room_id` (filtering + rollups) and touches nothing else; the EXPORT prints `room_area`, the free text (`export.py:114`/`:435`). Assigning without the second half would file every line and still hand the carrier a blank Room/Area column. So it also PATCHes the text per row — only rows whose text disagrees, chunked 10 at a time. **Unfile** clears the link and leaves the text (the words are the adjuster's, and they are what exports). See ask 28 for the server-side fix. |
| 05 Worksheet | **Delete N** (selection bar) | ✅ LIVE | `DELETE /v1/claim_items` with `item_ids`. Two-step: the button arms a confirm, Cancel changes nothing. Always available — it is the customer's data (rule 15). The confirm reassures that photos survive (`photos_detached` is reported when non-zero, rule 22: evidence is excluded, never destroyed) and, **only on a claim with `exported_at` set**, warns that lines below will renumber, because line numbers are positions on both surfaces and the carrier's copy cites the old ones (ask 29). |
| 77 Holdback recovery | **Whole screen** (`RecoveryPage`, `/claims/:id/recovery`) | ✅ LIVE | Post-settlement worksheet: actually-spent + units + receipt per line, `recoverable` read verbatim from the payload (services/holdback.py owns the lesser-of math). Gated to closed/archived; an open claim gets an explainer, not an empty table. Amounts and counts stay PAIRED — the count auto-fills, because an amount without one recovers $0 on money genuinely owed. |
| 77 | **Request · .xlsx / · PDF** | ✅ LIVE | `GET /claims/{id}/holdback-export`. **Only these two formats** — the design's `receipts` and `zip` are a 422. Shown only when a line qualifies (mirrors the server's `is_recoverable_line`, incl. NULL `replaced_qty` = whole line, explicit `0` = excluded); otherwise the bar says why. Repeatable by design and **never stamps `exported_at`** — verified. A claimed line with no receipt prints MISSING, and the screen warns first. |
| 13 Exports | **Whole screen** (`ExportsPage`, `/exports`) — nav tab now live | ✅ LIVE | Lists claims with `exported_at`, with re-download of both documents. **The design's ledger is deliberately NOT built**: there is no exports table and no export id / version / size / per-file status — only one first-write-wins timestamp (ask 30). The page says in words that a re-pulled file is rebuilt from the claim as it stands now, so an edited claim will not match the carrier's copy. The Recovery button's `409` renders as a sentence, not an error code. |
| 12 Claim overview | **From phone** → `PairPhoneModal` | ✅ LIVE | `POST /claims/{id}/pair-token` mints a single-use ~2-min token, rendered as a REAL QR (`qrcode-generator`, drawn as one SVG path). The token goes in the URL **fragment** — never the query string — because it is a bearer credential and a fragment is never logged or sent in a Referer. Countdown reads `expires_at` off the wall clock, so a backgrounded tab cannot show time the token does not have. Also shown grouped in fours to type by hand. Re-minting is free and invalidates nothing. |
| 12 | **Sign out paired phones** | ✅ LIVE | `POST /claims/{id}/pair-token/revoke`. Kills live CAPTURE credentials — it cannot recall an unredeemed code, and the zero-case copy says so rather than implying a recall. Idempotent; `revoked: 0` reads as a normal answer. Two-step confirm. |
| 26/27 Mobile pair | `/pair` (PUBLIC) | ✅ LIVE | Auto-redeems the token from the URL **fragment**, then `history.replaceState`s it out of the address bar so a credential-shaped value does not sit in history. Falls back to typing the code. One failure message for unknown/expired/used — the API returns one `401` so a caller cannot probe, and the UI does not invent a distinction it was denied. |
| 11 Mobile capture | `/capture` (PUBLIC) | ✅ LIVE | Built: shoot via the phone's own camera, per-BATCH room (captured per shot, batched on change), per-photo note (300), real queue states, honest "safe to leave". Verified end to end after the CORS fix (`2c2d030`): photos stored, per-batch room applied, a room change batched separately, the note saved on the right photo, and a credential used against another claim refused with `403`. Deliberately NOT ported: live viewfinder (costs EXIF the clusterer needs), flash, SCAN BARCODE (Chrome-only), the offline-queue banner (not built — so nothing claims it), and the artboards' fake phone chrome. |
| 28 Mobile review | Review, from the camera | ✅ LIVE | Grouped by room, untagged last. **Reads local session state** — the capture credential cannot `GET /staging`, so the footer says this is what this phone sent in this session, not the claim's staging set. Capture time is the FILE's `lastModified`, not the upload time. **A room can be fixed after upload** via the per-photo PATCH, which turns the design's "No room" pill into a fix. The design's **Blurry?** (on-device focus score) and **Won't process** (a staging reclassification the credential cannot make) are deliberately absent. |
| 11 Mobile capture | **Offline queue** | ✅ LIVE | Photos go to IndexedDB the moment they are taken and retry when the page is open and the network returns — they survive a reload, a crash and a lock screen. **Not Background Sync**: `SyncManager` does not exist on iOS Safari, so the copy promises only what is true — saved on the phone, sends itself when signal returns, nothing uploads while the screen is closed. Backoff 2s→30s; a dead credential, wrong claim or oversized file never auto-retries. **Success is read from the ack, not the status code** — a `202` accepts the request, not the photos, and only files the server did not refuse are treated as safe (a `duplicate` counts as landed, rule 21). A refused photo stays on the phone. |
| 11 Mobile capture | **Offline shell** (`public/sw.js`) | ✅ LIVE | `/capture` loads with no signal. Same-origin GETs only — the API is another origin and nothing of it is ever cached (verified: with the server stopped, API calls still hit the network and the cache holds no `railway.app` entry). Navigations network-first so a deploy lands on the next load; hashed `/assets/` cache-first since Vite fingerprints them. **No `skipWaiting()`** — a new worker waits for tabs to close rather than swapping the bundle under someone holding queued photos. `__kevinUnregisterSW()` is the console escape hatch, so support never has to say "clear site data" and destroy the photo queue. Registered in production only. |
| 31 Settings profile | `/settings/profile` | ✅ LIVE (read-only) | Renders the real identity and usage from `GET /v1/me`. **No save bar** — `/v1/me` is read-only and there is no profile write route (ask 33), so the page names the missing endpoint rather than showing a Save that lies. Explains that "Prepared by" is per-claim by design (ask 25): a point-in-time document keeps the details that were true when it was filed. |
| 32 Settings business | `/settings/business` | ✅ LIVE (explanatory) | No endpoint exists at all. States where the two fields that DO reach a carrier come from — preparer and firm, both per claim — and that tax follows the loss address, not the firm. |
| 33 Settings export defaults | `/settings/export` | ✅ LIVE (read-only) | Renders the **live depreciation schedule** from `GET /v1/depreciation-rules`, read-only: rule 13 says fetch it, never retype it, and an editable copy would be a second set of rules that can drift from the engine. `null` useful life prints "manual only", never "0 years". Also states the fixed Xactimate column order (rule 18) and that every exported cell is a static value, never a formula. |
| 34 Settings Xactimate | `/settings/integrations` | ✅ LIVE (explanatory) | **No Connect button, deliberately.** .xlsx not XML (rule 2), and Kevin never pushes into a carrier system (rule 4) — a connector would imply both things the product declines to do. Explains the hand-off and links to Exports. XactAnalysis delivery is named as a product decision, not a missing wire. |
| 36 Settings API | `/settings/api` | ✅ LIVE (explanatory) | No key issuance, list, rotation or webhook registry exists. Explains that the two narrow credentials which DO exist (capture token, share link) are not general-purpose keys, and that a key needs the same issue-once / hash / revoke / scope shape before it can exist. |
| Settings sidebar | Carrier profiles (10) · Pricing (14) | 🚫 disabled rows | Designed, unbuilt. Render as disabled spans (`.k-side-item--todo`), matching the `.k-claim-tab--todo` convention, so nothing in the sidebar is a dead click. **Billing (35) is owned elsewhere** and links out untouched. |
...
| 48 Sample-claim | **Sample claim** (footer, every page) · **Open the sample claim** (`/demo`, `/done-for-you`) · **See a finished claim** (landing hero) · **See a sample inventory** (`/for-estate-liquidators`) | 🔌 | All four are inert `<span class="k-mkt-soon">` — no href, so nothing 404s. BLOCKED on an unauthenticated read path: see `BACKEND-ASKS.md` #37. When `GET /v1/claims/sample` answers without a token, these become `<Link to="/sample">` and `/sample` redirects to `/claims/sample`, which renders through the worksheet that already exists. Do NOT unblock them by adding a seed-data mode to `WorksheetPage.tsx` (another agent's file) or by building a second grid (CLAUDE.md: there is ONE worksheet) |
| 15 Request-access | **Request a quote →** (submit) | ✅/🔌 | No `/v1/enterprise-lead` endpoint exists. The React port composes the nine answers — agency, contact, carriers, volume, note — into a `mailto:kevin@kevin.co` body rather than throwing them away the way a bare mailto would. When the endpoint lands, POST instead and keep the mailto as the failure path |
| 15 Request-access | form fields | ✅ | Controlled inputs, empty. The prototype ships them pre-filled with Marcus Delgado of Delgado & Co. — seed data that, live, hands a visitor a stranger's details to submit. The testimonial from that firm stays: approved copy, not a form value |
| 21 Pricing / 15 | **Talk to us about your desk** (Enterprise tier) | ✅ | → `/request-access`. Pro's tier CTA goes to `/sign-up`; neither is a sign-IN |
...
| 34 Xactimate | **Download sample** ×3 | ✅ Real files, `public/samples/`, generated by `scripts/build-samples.mjs`. Plain `<a download>` — no handler to fail, and middle-click/save-as behave. Money is computed with the server's chain so a sample cannot disagree with the product; the .xlsx carries static values only (rule 2b) and one row is deliberately unpriced (rule 12) |
| 34 / 36 / 32 / 35 | **Talk to us about Enterprise**, **See/Open API docs** | 🔌 Point at `/contact` and `/docs`, which the marketing stream owns and has not landed yet — they 404 today and will resolve when those routes ship. Targets are correct; deliberately not worked around here to avoid colliding with that port |
| 36 API | Plan comes from `quota.plan`, not a prop default | It defaulted to 'pro' for every account, so a real Enterprise customer was told they were on Pro and never reached their keys |
| 36 API | Enterprise keys/webhooks render an honest empty state | No key or webhook route exists (ask 36). The design's `sk_live_...` and example-tpa.com endpoints are NOT reproduced — a fabricated API key shown to a paying customer is worse than fake card digits |
| 36 API | **Copy** on the curl sample | ✅ Works, with a fallback: clipboard API, then selection-based copy, then "Press Ctrl+C" with a note. A denied clipboard previously produced an unhandled rejection and a button that did nothing |
