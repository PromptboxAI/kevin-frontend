...
| 11/26/27/28 Mobile | Capture surface | 🔌 next | Phone redeems at `POST /v1/pair` (public, rate-limited) and uploads with `X-Capture-Token` — deliberately not a Bearer, so an upload-only credential cannot look like a session. This is the flow that must send the per-batch `room` (see Rooms / ask 28). |
