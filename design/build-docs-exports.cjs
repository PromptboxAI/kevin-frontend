...
// kevin.co serves /docs out of the React app (src/pages/DocsPage.tsx), which
// cannot read a `window.X = X` .jsx file. Hand-copying the articles into src/
// would recreate exactly the drift this script exists to remove -- so the app
// gets a GENERATED module from the same source as the other two targets. The
// renderer lives in src/; only the content is generated.
function buildAppModule(nav, articles, counts) {
  ...
  return [
    ...
    '',
    `export const DOC_COUNTS = { articles: ${counts.articles}, sections: ${counts.sections} }`,
    '',
    'export const DOC_NAV: DocNavSection[] = ' + j(nav),
    '',
    'export const DOC_ARTICLES: Record<string, DocArticle> = ' + j(articles),
    '',
  ].join('\n');
}
...
function main() {
  ...
  const outputs = new Map([
    ['Kevin-docs.md', buildSingleFile(nav, articles, counts)],
    [path.join('..', 'src', 'content', 'docs-content.generated.ts'),
      buildAppModule(nav, articles, counts)],
  ]);
